...
ATLAS-Evo is organized as an evidence-tiered benchmark. Its purpose is to keep independent context evidence separate from related-branch support, prediction-derived capacity checks, boundary cases, and control analyses. The repository is intended for reproducibility and inspection of the benchmark workflow, not as a public archive of submission files.

## Contents

- `src/atlas_evo/`: reusable Python utilities for sequence features, metrics, evidence-tier checks, duplicate audit, ablation summaries, uncertainty summaries, and target-permutation controls.
